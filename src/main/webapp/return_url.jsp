	...
	if(verify_result){//验证成功
		...
		if(trade_status.equals("TRADE_FINISHED") || trade_status.equals("TRADE_SUCCESS")){
			//判断该笔订单是否在商户网站中已经做过处理
				...
				//如果有做过处理，不执行商户的业务程序
				//made by YRee
				//在这里转发到确定充值成功,只需要交易号和指令号
			request.getRequestDispatcher("recharge/confirmRecharge").forward(request, response);
		...
		//该页面可做页面美工编辑
		out.println("订单号是："+out_trade_no);
		out.println("<br />支付宝交易号是："+trade_no);
		out.println("<br />交易状态是："+trade_status);
		out.println("<br />验证成功<br />");
